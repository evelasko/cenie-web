  ...
  return (
    <>
      <Bidding title="Buy crypto" items={steps} activeIndex={activeIndex}>
        ...
        {activeIndex === 1 && (
          <EnterAmount goBack={() => setActiveIndex(0)} goNext={() => setActiveIndex(2)} />
        )}
        {activeIndex === 2 && (
          <PaymentInfo goBack={() => setActiveIndex(1)} goNext={() => setActiveIndex(3)} />
        )}
        {activeIndex === 3 && (
          <ConfirmOrder goBack={() => setActiveIndex(2)} goStart={() => setActiveIndex(0)} />
        )}
      </Bidding>
